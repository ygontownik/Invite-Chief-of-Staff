...
# ── Dashboard briefing file ───────────────────────────────────────────────────

def write_briefing_file(briefing: str) -> None:
    """Write briefing to /tmp so the dashboard briefing page can display it."""
    date_str = datetime.now().strftime("%Y-%m-%d")
    path = Path(f"/tmp/daily_briefing_{date_str}.txt")
    try:
        path.write_text(briefing, encoding="utf-8")
        log.info(f"Briefing written to {path}")
    except Exception as e:
        log.warning(f"Could not write briefing file: {e}")
...
def _markdown_to_html(md: str) -> str:
    ...
    for line in lines:
        # Headings
        if line.startswith("## "):
            line = f"<h2 style='color:#1a1a1a;margin:20px 0 6px'>{line[3:]}</h2>"
        elif line.startswith("### "):
            line = f"<h3 style='color:#333;margin:14px 0 4px'>{line[4:]}</h3>"
        # Bold
        line = re.sub(r"\*\*(.+?)\*\*", r"<strong>\1</strong>", line)
        # Italic
        line = re.sub(r"_(.+?)_", r"<em>\1</em>", line)
        # Inline code
        line = re.sub(r"`(.+?)`", r"<code style='background:#f4f4f4;padding:1px 4px'>\1</code>", line)
        # Bullet lines
        if line.startswith("- "):
...
padding:24px;color:#1a1a1a;line-height:1.55'>
{body}
<hr style='margin-top:32px;border:none;border-top:1px solid #ddd'>
<p style='color:#999;font-size:12px'>
  Generated by COS Pipeline · <a href='http://localhost:7777'>Dashboard</a>
</p>
</body></html>"""


def send_briefing_email(token: str, briefing: str, today_str: str) -> None:
    """Send the briefing as HTML email via Gmail API."""
    import base64
    import email.mime.multipart
    import email.mime.text

    fc  = _fc.load_firm_context()
    to  = ((fc.get("personal") or {}).get("daily_briefing") or {}).get("delivery_email") or ""
    if not to:
        to = (_fc._principal(fc).get("email") or "")
    if not to:
        log.warning("No delivery_email configured — skipping email send.")
        return

    subject = f"Briefing — {today_str}"
    html    = _markdown_to_html(briefing)

    msg = email.mime.multipart.MIMEMultipart("alternative")
    msg["Subject"] = subject
    msg["To"]      = to
    msg["From"]    = "me"
    msg.attach(email.mime.text.MIMEText(briefing, "plain"))
    ...
    try:
        with urllib.request.urlopen(req, timeout=20) as r:
            r.read()
        log.info(f"Briefing emailed to {to}")
    except urllib.error.HTTPError as e:
        body_err = e.read().decode("utf-8", errors="replace")
        if "insufficientPermissions" in body_err or "forbidden" in body_err.lower():
            log.warning(
                "Gmail send permission denied — add 'gmail.send' scope and re-run OAuth. "
                "Delete ~/credentials/token.json then run: python3 setup_oauth.py"
            )
        else:
            log.warning(f"Gmail send failed ({e.code}): {body_err[:200]}")
    except Exception as e:
...
{briefing_tail}"""
    ...
    # Write to /tmp so dashboard briefing page can display it
    write_briefing_file(briefing)

    # Send via email
    send_briefing_email(token, briefing, today_str)
