...
def build_system_prompt(ctx: dict) -> str:
    ...
    parts = [
        ...
        f"    pitched IN (the email-direction inversion failure mode: a fundraising\n"
        f"    advisor sends us a teaser → the row must read 'advisor sends teaser\n"
        f"    to {p_first}', NOT '{p_first} sends teaser to advisor').\n"
        ...
        f"    Example: 'set up working session between AcmeCo and John' →\n"
        f"    who='{p_first}', what='Set up working session: AcmeCo technical team\n"
        f"    + John Smith (DealCo) to review project sites'.\n"
