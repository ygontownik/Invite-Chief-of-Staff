...
# Document name for the calls doc in Drive (kept for legacy cmd_list)
CALLS_DOC_NAME     = "Call Transcripts & Memos"

# Individual call docs go here (Podcast/Call Transcripts folder)
TRANSCRIPTS_FOLDER_ID = "1B7UgpFCElgyZMLbq1yrf-N7PsB-UA4SE"
...
def create_call_doc(drive_svc, title: str, start_time: "datetime") -> str:
    """Create a new individual Google Doc for a single call in Transcripts folder."""
    import re
    date_label = start_time.strftime("%b %d %Y")
    safe_title = re.sub(r'[\\/*?:"<>|]', "-", title)
    doc_name   = f"{safe_title} ({date_label})"
    meta = {
        "name":     doc_name,
        "mimeType": "application/vnd.google-apps.document",
    ...
    doc_id = drive_svc.files().create(body=meta, fields="id").execute()["id"]
    print(f"    -> Created individual call doc: '{doc_name}'")
    return doc_id


def write_individual_call_doc(docs_svc, doc_id: str,
                               call_title: str, start_time: "datetime",
                               memo: str, transcript_block: str):
    """Write memo + transcript to a fresh individual call doc (no TOC)."""
    ...
    batch_update(docs_svc, doc_id, [
        {"insertText": {"location": {"index": 1}, "text": full_block}}
    ])
    # Style heading
    content = get_content(docs_svc, doc_id)
    apply_para_style(docs_svc, doc_id, content, ep_heading, "HEADING_1")
    # Style memo section headers
    memo_headers = [
...
def _process_recording(state: dict):
    ...
    # 4. Save to Google Drive — one doc per call in Transcripts folder
    print("  -> Saving to Google Drive...")
    try:
        drive_svc, docs_svc = get_services()

        doc_id = create_call_doc(drive_svc, title, start_time)
        write_individual_call_doc(docs_svc, doc_id, title, start_time, memo, transcript_block)

    except Exception as e:
        print(f"ERROR  Drive save failed: {e}")
        traceback.print_exc()
        # Save locally as fallback
        fallback_path = audio_path.replace(".mp3", "_transcript.txt")
        with open(fallback_path, "w") as f:
            f.write(f"MEMO\n{'='*60}\n{memo}\n\n{'='*60}\n\n{transcript_block}")
        print(f"    WARNING  Saved locally as fallback: {fallback_path}")
        return
    ...
    # Mark processed
    import re as _re
    call_id = _re.sub(r"[^a-zA-Z0-9_]", "_", f"{title}_{start_time.isoformat()}")[:120]
    ...
    # Hook will also move doc to Transcripts/_Ready/ so Drive organizer routes it.
    try:
        subprocess.Popen(
            [sys.executable, os.path.expanduser("~/cos-pipeline/cos_transcript_hook.py"),
...
def cmd_list(args):
    ...
    print(f"\n  Transcripts folder:")
    print(f"  https://drive.google.com/drive/folders/{TRANSCRIPTS_FOLDER_ID}")
