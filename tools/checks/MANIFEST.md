...
| `check_pi1.py`                  | PI1          | Any deal with a `project_url` in `sync-state.json` (online — `/capture-deal-chats` scrapes it) has no `project_instructions.doc_id` in `drive-docs.yaml`. Without this, `/refresh-project-instructions` silently skips the deal and it never receives the SESSION START PROTOCOL. Proven by freedom_fuels + fit missing instructions despite active chat capture. |
