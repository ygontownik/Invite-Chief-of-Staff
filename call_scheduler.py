...
def _load_principal_email() -> str:
    """Read principal.email from firm_context.yaml.
    Searches COS_CONFIG_DIR, any cos-pipeline-config-<slug>/, then legacy locations."""
    candidates = []
    env = os.environ.get("COS_CONFIG_DIR")
    if env:
        candidates.append(Path(env).expanduser() / "firm_context.yaml")
    for cand in sorted(Path.home().glob("cos-pipeline-config-*")):
        candidates.append(cand / "firm_context.yaml")
    candidates += [
        Path.home() / "cos-pipeline-config" / "firm_context.yaml",
    ...
    for p in candidates:
        if p.exists():
            try:
                ...
                pass
    raise RuntimeError(
        "Could not load principal.email from firm_context.yaml. "
        "Set COS_CONFIG_DIR in ~/.zshrc or ensure firm_context.yaml exists "
...
# Paths — call_recorder.py lives alongside this script in cos-pipeline by default;
# CALL_RECORDER_PATH env var overrides for split-repo setups.
def _resolve_recorder_script() -> str:
    env = os.environ.get("CALL_RECORDER_PATH", "")
    if env and os.path.exists(os.path.expanduser(env)):
        return os.path.expanduser(env)
    for cand in (
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "call_recorder.py"),
        os.path.expanduser("~/cos-pipeline/call_recorder.py"),
        os.path.expanduser("~/scripts/call_recorder.py"),
    ):
        if os.path.exists(cand):
            return cand
    return ""  # caller checks for empty before invoking
RECORDER_SCRIPT = _resolve_recorder_script()
...
_NOTIFY_TO        = _load_principal_email()
...
def _send_confirmation_email(title: str, start_at: datetime, end: datetime, source: str):
    ...
    try:
        ...
        body = (
            f"Meeting detected: {title}\n\n"
            f"Starts : {local_start.strftime('%a %b %-d at %-I:%M %p %Z')}\n"
            f"Ends   : {local_end.strftime('%-I:%M %p %Z')}\n"
            f"Source : {source or 'Google Calendar'}\n"
            f"Note   : Auto-recording not yet configured (see Follow-ups row 424)\n"
        )
...
def schedule_meeting(meeting: dict):
    ...
    start_label = f"com.cos.recorder.start.{safe_id}"
    stop_label  = f"com.cos.recorder.stop.{safe_id}"

    # TODO (row 424): wire to call_recorder.py once Aggregate Device is set up
    # _write_launchd_plist(label=start_label, program=..., args=[RECORDER_SCRIPT, "start", ...], ...)
    # _write_launchd_plist(label=stop_label,  program=..., args=[RECORDER_SCRIPT, "stop"],      ...)
