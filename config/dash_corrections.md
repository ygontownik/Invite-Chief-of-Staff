...
### 2026-05-21 — `claude-task` plists need `/opt/homebrew/bin` BEFORE `/usr/bin` on PATH

The `com.yoni.claude-task.*.plist` files at `~/Library/LaunchAgents/`
shipped with `EnvironmentVariables.PATH =
/usr/local/bin:/usr/bin:/bin:/opt/homebrew/bin:/Users/ygontownik/.local/bin`.
That puts `/usr/bin/python3` (3.9) AHEAD of `/opt/homebrew/bin/python3`
(3.14), so any SKILL that invokes `python3 ~/cos-pipeline/foo.py` lands
on 3.9 — and any script requiring `claude_agent_sdk` (Python ≥ 3.10)
fails immediately with `claude_agent_sdk not installed`.

This silently caused `cos-personal-briefing` to fail in subscription
mode after flipping `firm_context.yaml :: auth_mode` from `api` to
`subscription`. The API-mode path doesn't need the SDK, so the bug
hid for as long as the tenant was on `api`.

**Rule**: PATH in `EnvironmentVariables` of every
`com.yoni.claude-task.*.plist` must put homebrew bin first:
`/opt/homebrew/bin:/Users/ygontownik/.local/bin:/usr/local/bin:/usr/bin:/bin`.
These plists are NOT generated by `~/dashboards/config/launchd-plists/generate.sh`
(only the `com.tomaccove.*` ones are) — they are Claude Code's own
creations, so edit them directly. After editing, `launchctl bootout`
then `launchctl bootstrap` each one to pick up the new env.

**Detection**: `grep -L "opt/homebrew/bin:/Users" ~/Library/LaunchAgents/com.yoni.claude-task.*.plist`
prints any plist whose PATH ordering is wrong.
...
## TEMPLATE SYNC — The running server reads from cos-pipeline/templates/, NOT dashboards/app/templates/ (2026-05-26) [SUPERSEDES 2026-05-25 entry below]

The actual template the RUNNING SERVER uses is at `/Users/ygontownik/cos-pipeline/templates/`:
- SOURCE: `cos-dashboard.template.html` — edit this to change the card/layout JS
- RENDERED: `cos-dashboard.rendered.html` — written by POST /refresh (calling `cos-dashboard-refresh.py`)

Files at `/Users/ygontownik/dashboards/app/templates/` are NOT served by the live server. Editing those files has ZERO effect on the dashboard.

**Correct edit workflow for JS/layout changes:**
1. Edit `/Users/ygontownik/cos-pipeline/templates/cos-dashboard.template.html` (Python replace for 4MB+ files)
2. Call `POST /refresh` — this runs `cos-dashboard-refresh.py` which reads the template, writes `cos-dashboard.rendered.html`
3. Verify with `curl ... | grep <marker>`

**Why "Pull fresh data" reverts changes:** The warmup calls the refresh script which reads from the template. If the template had the old card, every warmup/pull-fresh-data would restore it.

## TEMPLATE SYNC — Edits must go to both cos-dashboard.html AND cos-dashboard.rendered.html (2026-05-25) [SUPERSEDED 2026-05-26]
The server serves `app/templates/cos-dashboard.rendered.html` (COS_DASHBOARD_RENDERED), not
`cos-dashboard.html`. Edits to the `.html` file are invisible to the running server. Both files
carry identical JS/HTML — always edit both. The `.rendered.html` is gitignored, so only
`.html` is committed; the rendered file is a build artifact regenerated by `_inject_shared_chrome()`.
Going forward: edit `.html` (committed), then manually mirror to `.rendered.html` before bouncing.
